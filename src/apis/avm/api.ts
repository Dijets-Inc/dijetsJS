...
import { Defaults, PrimaryAssetAlias, ONEDJTX } from "../../utils/constants"
...
export class AVMAPI extends JRPCAPI {
  ...
  getDJTXAssetID = async (refresh: boolean = false): Promise<Buffer> => {
    if (typeof this.DJTXAssetID === "undefined" || refresh) {
      const asset: GetDJTXAssetIDParams = await this.getAssetDescription(
        PrimaryAssetAlias
      )
  ...
  checkGooseEgg = async (
    ...
    const fee: BN = utx.getBurn(djtxAssetID)
    if (fee.lte(ONEDJTX.mul(new BN(10))) || fee.lte(outputTotal)) {
      return true
    } else {
      return false
    }
  ...
  buildBaseTx = async (
    ...
    const feeAssetID: Buffer = await this.getDJTXAssetID()
