...
export class ImportTx extends EVMBaseTx {
  ...
  private validateOuts(fee: BN): void {
    ...
    const djtxAssetID: string =
      Defaults.network[`${selectedNetwork}`].X.djtxAssetID
    // sum incoming DJTX
    ...
    if (feeDiff.lt(fee)) {
      const errorMessage: string = `Error - ${fee} nDJTX required for fee and only ${feeDiff} nDJTX provided`
