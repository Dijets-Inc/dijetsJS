...
export class EVMAPI extends JRPCAPI {
  ...
  protected DJTXAssetID: Buffer = undefined
   ...
   */
  getDJTXAssetID = async (refresh: boolean = false): Promise<Buffer> => {
   ...
   * @returns The the provided string representing the DJTX AssetID
   */
  setDJTXAssetID = (djtxAssetID: string | Buffer) => {
  ...
  issueTx = async (tx: string | Buffer | Tx): Promise<string> => {
    ...
    if (typeof tx === "string") {
      ...
      throw new TransactionError(
        "Error - djtx.issueTx: provided tx is not expected type of string, Buffer, or Tx"
      )
    }
  ...
  buildImportTx = async (
    ...
    const djtxAssetIDBuf: Buffer = bintools.cb58Decode(djtxAssetID)
  ...
  buildExportTx = async (
    ...
    const assetDescription: any = await this.getAssetDescription("DJTX")
    ...
    if (bintools.cb58Encode(assetDescription.assetID) === assetID) {
      ...
      const evmDJTXInput: EVMInput = new EVMInput(
        fromAddressHex,
        fee,
