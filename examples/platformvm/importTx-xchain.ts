...
const memo: Buffer = Buffer.from(
  "Manually Import DJTX to the P-Chain from the X-Chain"
)

const main = async (): Promise<any> => {
  const djtxAssetID: Buffer = await pchain.getDJTXAssetID()
  ...
  utxos.forEach((utxo: UTXO) => {
    ...
    if (djtxAssetID.toString("hex") === assetID.toString("hex")) {
