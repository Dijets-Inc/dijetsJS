...
import { ONEDJTX } from "../../../src/utils/constants"
import {
  Serializable,
  Serialization,
  SerializedEncoding,
...
describe("PlatformVMAPI", (): void => {
  ...
  test("exportDJTX", async (): Promise<void> => {
    ...
    const result: Promise<string | ErrorResponseObject> = api.exportDJTX(
      username,
      password,
      amount,
      to
  ...
  test("importDJTX", async (): Promise<void> => {
    const to: string = "abcdef"
    const username: string = "Robert"
    const password = "Paulson"
    const txID = "valid"
    const result: Promise<string | ErrorResponseObject> = api.importDJTX(
      username,
      password,
      to,
      blockchainID
  ...
  describe("Transactions", (): void => {
    ...
    beforeEach(async (): Promise<void> => {
      ...
      const result: Promise<Buffer> = platformvm.getDJTXAssetID()
      const payload: object = {
        result: {
          name,
          symbol,
      ...
      const amount: BN = ONEDJTX.mul(new BN(amnt))
    ...
    test("signTx", async (): Promise<void> => {
      const assetID: Buffer = await platformvm.getDJTXAssetID()
    ...
    test("buildImportTx", async (): Promise<void> => {
      ...
      const txu2: UnsignedTx = set.buildImportTx(
        ...
        platformvm.getTxFee(),
        await platformvm.getDJTXAssetID(),
    ...
    */
    ...
    test("buildAddValidatorTx sort StakeableLockOuts 1", async (): Promise<void> => {
      ...
      const assetID = await platformvm.getDJTXAssetID()
      const assetID2 = await platformvm.getDJTXAssetID()
    ...
    test("buildAddValidatorTx sort StakeableLockOuts 2", async (): Promise<void> => {
      ...
      const outputidx1: number = 0
      const assetID: Buffer = await platformvm.getDJTXAssetID()
      const assetID2: Buffer = await platformvm.getDJTXAssetID()
      const utxo1: UTXO = new UTXO(
        codecID,
        txid,
        outputidx0,
    ...
    test("buildAddValidatorTx 2", async (): Promise<void> => {
      ...
      const addrbuff2 = addrs2.map((a) => platformvm.parseAddress(a))
      const addrbuff3 = addrs3.map((a) => platformvm.parseAddress(a))
      const amount: BN = ONEDJTX.mul(new BN(25))

      const locktime: BN = new BN(54321)
      const threshold: number = 2

      platformvm.setMinStake(ONEDJTX.mul(new BN(25)), ONEDJTX.mul(new BN(25)))
    ...
    test("buildAddValidatorTx 3", async (): Promise<void> => {
      ...
      const addrbuff2 = addrs2.map((a) => platformvm.parseAddress(a))
      const addrbuff3 = addrs3.map((a) => platformvm.parseAddress(a))
      const amount: BN = ONEDJTX.mul(new BN(3))

      const locktime: BN = new BN(54321)
      const threshold: number = 2

      platformvm.setMinStake(ONEDJTX.mul(new BN(3)), ONEDJTX.mul(new BN(3)))
