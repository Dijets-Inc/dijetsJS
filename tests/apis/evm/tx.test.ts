...
describe("EVM Transactions", () => {
  describe("ImportTx", () => {
    test("Multiple DJTX EVMOutput fail", (): void => {
      const outputidx: Buffer = Buffer.from("")
      const input: SECPTransferInput = new SECPTransferInput(ONEDJTX)
      const xferin: TransferableInput = new TransferableInput(
        bintools.cb58Decode(txID),
        outputidx,
        bintools.cb58Decode(djtxAssetID),
        input
      )
      ...
      evmOutput = new EVMOutput(cHexAddress1, ONEDJTX, djtxAssetID)
      evmOutputs.push(evmOutput)
    ...
    test("Multiple DJTX EVMOutput success", (): void => {
      const outputidx: Buffer = Buffer.from("")
      const input: SECPTransferInput = new SECPTransferInput(ONEDJTX)
      const xferin: TransferableInput = new TransferableInput(
        bintools.cb58Decode(txID),
    ...
    test("Multiple ANT EVMOutput fail", (): void => {
      ...
      evmOutputs.push(evmOutput)
      evmOutput = new EVMOutput(cHexAddress1, ONEDJTX, antAssetID)
      evmOutputs.push(evmOutput)
      expect((): void => {
        new ImportTx(
          networkID,
    ...
    test("Multiple ANT EVMOutput success", (): void => {
      ...
      evmOutputs.push(evmOutput)
      evmOutput = new EVMOutput(cHexAddress2, ONEDJTX, antAssetID)
      evmOutputs.push(evmOutput)
    ...
    test("Single ANT EVMOutput fail", (): void => {
      ...
      expect((): void => {
        ...
        "Error - 25000000000 nDJTX required for fee and only 0 nDJTX provided"
      )
    })
    ...
    test("Single DJTX EVMOutput fail", (): void => {
      const outputidx: Buffer = Buffer.from("")
      const input: SECPTransferInput = new SECPTransferInput(new BN(507))
      const xferin: TransferableInput = new TransferableInput(
        bintools.cb58Decode(txID),
      ...
      expect((): void => {
        ...
        "Error - 25000000000 nDJTX required for fee and only 507 nDJTX provided"
      )
    })

    test("Single DJTX EVMOutput success", (): void => {
      const outputidx: Buffer = Buffer.from("")
      const input: SECPTransferInput = new SECPTransferInput(ONEDJTX)
      const xferin: TransferableInput = new TransferableInput(
        bintools.cb58Decode(txID),
