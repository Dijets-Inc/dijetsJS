...
describe("EVMAPI", (): void => {
  ...
  beforeAll((): void => {
    api = new EVMAPI(avalanche, "/ext/bc/C/djtx", blockchainID)
  })
  ...
  test("exportDJTX", async (): Promise<void> => {
    ...
    let result: Promise<string> = api.exportDJTX(username, password, to, amount)
    let payload: object = {
      result: {
        txID: txID
      }
  ...
  test("importDJTX", async (): Promise<void> => {
    ...
    let password: string = "Paulson"
    let txID: string = "valid"
    let result: Promise<string> = api.importDJTX(
      username,
      password,
      to,
      blockchainID
  ...
  test("refreshBlockchainID", async (): Promise<void> => {
    ...
    const testAPI: EVMAPI = new EVMAPI(avalanche, "/ext/bc/C/djtx", n5bcID)
