...
describe("BinTools", (): void => {
  ...
  test("stringToAddress", (): void => {
    // Missing prefix
    let addr: string = "-djtx13a4ye34zdfa33zeg3udnz533d6msfuqkds9hq7"
    expect((): void => {
      bintools.stringToAddress(addr)
    }).toThrow("Error - Valid address must have prefix before -")

    // Missing -
    addr = "Xdjtx13a4ye34zdfa33zeg3udnz533d6msfuqkds9hq7"
    expect((): void => {
      bintools.stringToAddress(addr)
    }).toThrow("Error - Valid address should include -")

    // Missing seperator (1)
    addr = "X-djtx3a4ye34zdfa33zeg3udnz533d6msfuqkds9hq7"
    expect((): void => {
      bintools.stringToAddress(addr)
    ...
    // Invalid HRP
    addr = "X-djtx11ycxp65vz60m87mkm2hsw3m5fadjlpldzntvr33"
    expect((): void => {
      bintools.stringToAddress(addr)
    ...
    addrs = [
      "X-djtx1dcfyuug87xqayl4fpp02z9dvknwhafdsjlw6m9",
      "X-djtx1ltghj033re64920k786uprcp82p9e36j8yl7hw",
      "X-djtx1dq4q9seql2spxrkd7rl82uck5ej3nvlh6vp3k5"
    ]
    addrs.forEach((address: string): void => {
      bintools.stringToAddress(address)
    })
    ...
    addrs = [
      "P-djtx1dcfyuug87xqayl4fpp02z9dvknwhafdsjlw6m9",
      "P-djtx1ltghj033re64920k786uprcp82p9e36j8yl7hw",
      "P-djtx1dq4q9seql2spxrkd7rl82uck5ej3nvlh6vp3k5"
    ]
    addrs.forEach((address: string): void => {
      bintools.stringToAddress(address)
    })
    ...
    addrs = [
      "C-djtx1dcfyuug87xqayl4fpp02z9dvknwhafdsjlw6m9",
      "C-djtx1ltghj033re64920k786uprcp82p9e36j8yl7hw",
      "C-djtx1dq4q9seql2spxrkd7rl82uck5ej3nvlh6vp3k5"
    ]
    addrs.forEach((address: string): void => {
      bintools.stringToAddress(address)
    })
